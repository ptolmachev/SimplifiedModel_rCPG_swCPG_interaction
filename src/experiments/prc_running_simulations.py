...
from src.exp_protocols.protocols import run_full_protocol
from src.utils.sp_utils import *
from src.construct_model import construct_model
from src.utils.gen_utils import create_dir_if_not_exist, get_project_root, modify_array
import json
import numpy as np
import pickle
...
def run_model_apply_stimulus(model, t_stim_start, stim_duration, stim_amp, t_stop):
    ...
    pnames = model.pnames
    model.run(t_stim_start, input=modify_array(external_inputs, pnames.index("Sensory_relay"), 0))
    model.run(stim_duration, input=modify_array(external_inputs, pnames.index("Sensory_relay"), stim_amp))
    model.run(t_stop - (t_stim_start + stim_duration), input=modify_array(external_inputs, pnames.index("Sensory_relay"), 0))
...
def run_simulations(experiment_params, model_params, stim_schedule_data, folder_save_to):
    ...
    for i in tqdm(range(len(stim_times))[::-1]):
        for j in range(len(t_insp_s)):
            ...
            recording_data['population_names'] = model.pnames
