...
def plot_analytics(base_folder, xlabel, title, x_vline, PIR_bnds, save_to):
    ...
    plt.grid(True)
    plt.savefig(os.path.join(img_folder, save_to), bbox_inch="tight")
    plt.savefig(os.path.join(img_folder, save_to.split(".svg")[0] + '.pdf'), bbox_inch="tight")
    plt.show()
    plt.close()
    return None
