...
model.W[model.pnames.index("Sensory_relay"), model.pnames.index("Insp")] = -0.22
model.W[model.pnames.index("Sensory_relay"), model.pnames.index("RampI")] = -0.22
...
# data["dt"] = model.dt
#
# save_to = os.path.join(get_project_root(), "data", "recordings_Relay_weakened.pkl")
# # save_to = os.path.join(get_project_root(), "data", "recordings_KF_inhibited.pkl")
# pickle.dump(data, open(save_to, "wb+"))

load_from = os.path.join(get_project_root(), "data", "recordings_Relay_weakened.pkl")
# load_from = os.path.join(get_project_root(), "data", "recordings_KF_inhibited.pkl")
...
axes[1].plot(VNA, linewidth = 2, c = 'b', label = "VNA")
for i in range(2):
    ...
    axes[i].axis('off')
plt.suptitle("Pathological breakthroughs in the Phrenic Nerve Activity", fontsize=15)
plt.subplots_adjust(wspace=0, hspace=0)
img_folder = os.path.join(get_project_root(), "img")
plt.savefig(os.path.join(img_folder, "pathological_breakthroughs.pdf"), bbox_inches='tight')
plt.savefig(os.path.join(img_folder, "pathological_breakthroughs.svg"), bbox_inches='tight')
plt.show()
plt.close()