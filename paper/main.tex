...
\textcolor{red}{The synaptic connectivity that our model posits is, at present, constrained only indirectly---by the requirement that it reproduce the observed motor patterns---and the connection weights are set by hand. Emerging conductance-inference techniques offer a route to constrain this connectivity empirically. \citet{molkov2025inference} introduced a method that decomposes single-neuron intracellular recordings in rhythmically active networks into phase-resolved excitatory and inhibitory synaptic conductances, applied it to identified respiratory interneurons in the \textit{in situ} rat brainstem, and \citet{molkov2026functional} extended it to genetically defined excitatory and inhibitory preBötC populations in the rhythmic slice. Because each population in our model is specified by the sign, targets, and strengths of its projections, such measurements could replace hand-tuning of these weights with data-derived estimates and, applied to neurons recorded during fictive swallowing, could test the specific breathing--swallowing connections the model predicts---for example, the SLN-driven inhibition of inspiratory premotor neurons or the pontine gating of the swallowing circuit. The same data also qualify one of our simplifications: whereas we represent the postinspiratory inhibition mediating glottal closure as a single pontine population, postinspiratory inhibitory neurons are in fact distributed across the preBötC and Bötzinger complex \citep{molkov2025inference}, so this drive likely arises from a more distributed source than the model assumes.}
