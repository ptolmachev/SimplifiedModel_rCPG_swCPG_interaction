...
\begin{align*}
 \tau_v \frac{dv}{dt} &= -v - m + (\text{drive} - b)+\text{input} \\
 \tau_m \frac{dm}{dt} &= f(v) - m,
\end{align*}
where $f(v) = 1/(1+e^{-s v} )$ - activity level of a population, $f(v) \in (0,1)$.
In these equations, the dimensionless variable v is analogous to the average membrane potential of a whole neural population. The variable $m$ mediates the firing rate (spike frequency) adaptation and acts on a much slower timescale than the variable $v$. To account for the correct timescale of the dynamics and the excitability of the neuronal populations, we use $b=0.2$, $\tau_v=2$~ms, $s=30$. The parameter $\tau_m$, which controls the timescale of firing rate adaptation, has distinct values for the different neuronal populations (discussed later), and the details are summarized in \Tableref{tbl: intrinsic_params}. The parameters are chosen manually to match the timescale of respiratory phase durations (inspiration, postinspiration, late expiration) of experimental recordings.
...
\begin{align*}
 \text{input}_j = \sum_{i \in C}^N w_{ij} f(v_i) +\text{stimulus}_j
\end{align*}
...
The variation of firing rate adaptation constant $\tau_m$ of the `$\text{Sw}_1$' population has a drastic effect on the number of produced sequential swallows (\Figref[A]{fig: sensitivity_analysis}). However, for the lower values ($\tau_m \leq 500$~ms), the unphysiologically high number of swallows (reaching up to 27 swallows during a 10~s input simulation) results from non-physiologically short swallow burst durations.

Changes in the arousal drive to the swallowing-related populations revealed that the the relationship between drive strength and number of swallows is not monotonic, reaching a peak of 8 produced swallows at about 80\% of the normal value of the drive, while the solitary swallowing response was abolished at the values lower than approximately 90\% (\Figref[B]{fig: sensitivity_analysis})

Variation of the inhibition from `$\text{Sw}_2$' to `$\text{Sw}_1$' revealed that the number of swallows in the Sw-CPG increases when the inhibition has lower absolute values (from $-0.6$ to $-0.2$) reaching as high as $35$ swallows. However, the width of these swallows was unphysiologically short (\Figref[C]{fig: sensitivity_analysis}). Moreover, reducing the inhibition of `$\text{Sw}_1$' by `$\text{Sw}_2$' eventually resulted in persistent pathological swallowing behavior (blue-shaded region).
...
\textcolor{red}{This emphasis on synaptic architecture over intrinsic biophysics has recently received direct experimental support. Using a conductance-inference technique that decomposes single-neuron intracellular recordings into their excitatory and inhibitory components, \citet{molkov2025inference} reconstructed the functional connectome of preBötC and Bötzinger complex interneurons in the \textit{in situ} rat brainstem, and \citet{molkov2026functional} extended this analysis to genetically identified excitatory and inhibitory populations of the rhythmically active medullary slice. These data-derived connectomes corroborate the central architectural assumption of our R-CPG core---a ring of mutually inhibiting inspiratory, postinspiratory, and late-expiratory populations \citep{smith2007spatial, rubin2009multiple}---by showing that inspiratory neurons receive phasic inhibition from postinspiratory and augmenting-expiratory populations and that inspiratory and expiratory populations interact through reciprocal inhibition. They also support our treatment of the ramping inspiratory discharge as a product of phasic inhibition rather than an intrinsic mechanism, since ramp-inspiratory neurons were found to receive sustained inhibition throughout the postinspiratory and late-expiratory phases. Notably, the isolated slice expresses only a reduced, two-phase organization lacking the multiphasic expiratory structure, which the authors attribute to the absence of pontine and Bötzinger complex inputs \citep{molkov2026functional}; this is consistent with our placement of postinspiratory activity under descending pontine control and with our finding that simulated pontine silencing collapses the three-phase pattern into apneusis. We note, however, that the inferred connectomes are richer than the minimal three-phase ring adopted here, comprising additional inhibitory connections and recurrent excitation within populations; our reduced connectivity is sufficient for the breathing--swallowing phenomena examined but is a deliberate simplification of this denser measured circuitry. These analyses concern the respiratory core only and do not constrain the swallowing circuit, for which comparable conductance-level data are not yet available.}

\textcolor{red}{A corollary of prioritizing synaptic connectivity is that the present model cannot, by construction, predict the effects of pharmacological or genetic manipulations that act on specific ion channels or receptors. The same degeneracy that justifies omitting intrinsic conductances---similar firing patterns arising from diverse channel compositions \citep{schulz2006variable, marder2007understanding, goaillard2009functional, marder2015robust}---also implies that the response to a receptor agonist or antagonist cannot be inferred from network connectivity alone. Advancing toward models that predict such manipulations will require channel- and receptor-resolved data tied to individual, phenotypically identified populations: voltage-clamp isolation and pharmacological dissection of the intrinsic currents implicated in respiratory rhythm and pattern generation (e.g. persistent sodium, calcium-activated nonspecific cation, and hyperpolarization-activated currents), per-population synaptic-conductance profiling of the kind described above \citep{molkov2025inference, molkov2026functional}, and dynamic-clamp validation of the predicted conductance combinations in the living neuron. Incorporating such data would allow detailed computational models to predict how targeting specific receptors in specific populations alters the integrated breathing--swallowing behavior, and thus to help guide pharmacological interventions for breathing--swallowing disorders. The network-level model presented here is a deliberately modest but necessary first step toward that goal.}
...
\begin{figure}
 \begin{center}
 \includegraphics[width=0.7\textwidth]{Figures/3_complete_model.pdf}
 \end{center}
...
\begin{figure}
 ...
 \end{center}
 \caption{A short 150~ms stimulation of the superior laryngeal nerve (depicted by grey shadings) elicits a response in the Sw-CPG. Here we isolate the swallowing CPG and provided an external stimulus from the `Sensory Relay' neurons to study the mechanisms underlying the generation of the single swallow in the swallowing half-center oscillator. Sub-figures (\textbf{A}) and (\textbf{C}) depict the simulated membrane-potential traces $v_1$, $v_2$ of the half-center oscillator (index 1 denotes the premotor swallowing `$\text{Sw}_1$' neurons); the numbered points (1--4 in the first scenario, 1--3 in the second) mark the successive stages of the trajectory referenced in the text, while the slow adaptation variables $m_1$, $m_2$ form the horizontal axes of the phase-space panels (\textbf{B}, \textbf{D}). The swallowing premotor neurons are strongly inhibited in the absence of the external input, however, are readily excitable.
...
\begin{figure}
 ...
 \end{center}
 \caption{\textbf{Control of the swallowing: variation of the parameters of the Sw-CPG.} To acquire each point on a graph, we simulated the network applying a long stimulation of `Sensory Relay' (Amplitude $A=0.45$, duration $T = 10$~s) to quantify the sequential swallowing response, and a short stimulation ($A=0.45$, $T=100$~ms) -- to probe the evoked solitary after-stimulus swallow visible in the simulated phrenic (PNA) and vagus (VNA) nerve responses (example traces 1 and 2, marked by the arrows on the analytics panel). The default value of the parameter is depicted by a grey solid line. Red- and blue-shaded regions highlight the parameter ranges where, respectively, the post-inhibitory-rebound solitary swallow and spontaneous swallows occur. See text for details.
...
Sw. gate & 5000 \\ \hline
\end{tabular}
\captionof{table}{Intrinsic parameters. For all populations: $b=0.2$, $\tau_v=2$~ms, $s=30$.}\label{tbl: intrinsic_params}
\end{center}
% \backmatter
